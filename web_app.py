...
def send_email(to_email, code, subject="Ersan Makine - Dogrulama Kodu"):
    ...
    <body style="font-family: Arial, sans-serif; color: #333;">
        ...
        <p>Merhaba,</p>
        <p>Sistem uzerinden yapilan islem icin gereken guvenlik kodunuz asagidadir:</p>
...
# =====================================================================
# BAGIMSIZ "users.db" VERITABANI YONETIMI
# =====================================================================
...
def init_advanced_b2b():
    exec_user_query("""CREATE TABLE IF NOT EXISTS users (
        ...
        user_type TEXT DEFAULT 'Satici', phone TEXT, 
    ...
    if not admin_check:
        exec_user_query("""INSERT INTO users (email, password, company_name, role, is_approved, is_verified, user_type) 
                           VALUES (?, ?, 'Ersan Makine Merkez', 'admin', 1, 1, 'Yonetici')""", 
...
if not st.session_state.logged_in:
    ...
    if current_token:
        ...
        if valid_user:
            ...
            st.session_state.user_role = u_role if u_role == 'admin' else ("manufacturer" if u_type == "Uretici" else "dealer")
    ...
    with col_main:
        st.markdown("""
            <div style='text-align: center; padding: 20px 0 10px 0;'>
                ...
                <p style='color: #64748b; font-size: 14px;'>Sisteme erismek icin bilgilerinizi giriniz.</p>
            </div>
        """, unsafe_allow_html=True)

        # Ikonlar kodla cagirildi (:key: vb.)
        tab_login, tab_register, tab_forgot = st.tabs([":key: Giris Yap", ":memo: Yeni Kayit", ":question: Sifremi Unuttum"])
        
        with tab_login:
            with st.container(border=True):
                le = st.text_input("E-Posta Adresi", placeholder="ornek@firma.com").strip().lower()
                lp = st.text_input("Sifre", type="password", placeholder="••••••••")
                rem = st.checkbox("Beni Hatirla", value=True)
                
                st.write("")
                if st.button("SISTEME GIRIS YAP", type="primary", use_container_width=True):
                    ...
                    if user:
                        ...
                        if ver == 0: st.warning(":warning: E-posta adresiniz dogrulanmamis!")
                        elif app == 0: st.warning(":hourglass_flowing_sand: Hesabiniz yonetici onayi bekliyor.")
                        else:
                            tok = str(uuid.uuid4())
                            ...
                            st.session_state.user_role = urole if urole == 'admin' else ("manufacturer" if utype == "Uretici" else "dealer")
                            st.rerun()
                    else: 
                        st.error("Hatali e-posta veya sifre!")

        with tab_register:
            with st.container(border=True):
                if st.session_state.reg_step == 1:
                    reg_type = st.selectbox("Faaliyet Turunuz", ["Satici (Bayi)", "Uretici"])
                    reg_comp = st.text_input("Firma Tam Unvani *", placeholder="Ersan Makine Ltd. Sti.")
                    reg_phone = st.text_input("Iletisim Numarasi *", placeholder="05XX XXX XX XX")
                    reg_email = st.text_input("Kurumsal E-Posta *", placeholder="ornek@firma.com").strip().lower()
                    reg_pwd = st.text_input("Sistem Sifresi Belirleyin *", type="password")
                    
                    st.write("")
                    if st.button("Kayit Ol ve Dogrula", use_container_width=True):
                        if all([reg_comp, reg_phone, reg_email, reg_pwd]):
                            if get_user_query("SELECT id FROM users WHERE email=?", (reg_email,)):
                                st.error("Bu e-posta adresi zaten kayitli!")
                            else:
                                ...
                                if send_email(reg_email, ver_code, "Uyelik Dogrulama Kodu"):
                                    st.session_state.temp_email = reg_email
                                    st.session_state.reg_step = 2
                                    st.rerun()
                        else:
                            st.warning("Lutfen (*) ile isaretli alanlari doldurun.")
                
                elif st.session_state.reg_step == 2:
                    st.success(f"{st.session_state.temp_email} adresine 6 haneli kod gonderildi.")
                    entered_code = st.text_input("Dogrulama Kodunu Giriniz", max_chars=6)
                    if st.button("Kodu Onayla", type="primary", use_container_width=True):
                        ...
                        if db_code and db_code[0][0] == entered_code:
                            ...
                            st.balloons()
                            st.success("Tebrikler! Hesabiniz dogrulandi.")
                        else:
                            st.error("Hatali kod girdiniz.")

        with tab_forgot:
            with st.container(border=True):
                if st.session_state.forgot_step == 1:
                    f_email = st.text_input("Kayitli E-Posta Adresiniz", placeholder="ornek@firma.com").strip().lower()
                    st.write("")
                    if st.button("Sifirlama Kodu Gonder", use_container_width=True):
                        if f_email and get_user_query("SELECT id FROM users WHERE email=?", (f_email,)):
                            ...
                            if send_email(f_email, reset_code, "Sifre Sifirlama Kodu"):
                                st.session_state.temp_email = f_email
                                st.session_state.forgot_step = 2
                                st.rerun()
                        else: st.error("Sistemde boyle bir e-posta bulunamadi.")
                
                elif st.session_state.forgot_step == 2:
                    st.info("E-postaniza gelen kodu girin.")
                    f_code = st.text_input("6 Haneli Kodu Girin", max_chars=6)
                    new_pwd = st.text_input("Yeni Sifreniz", type="password")
                    if st.button("Sifremi Degistir", type="primary", use_container_width=True):
                        ...
                        if valid_code and valid_code[0][0] == f_code and len(new_pwd) > 0:
                            ...
                            st.success("Sifreniz degistirildi! Giris yapabilirsiniz.")
                        else: st.error("Hatali kod veya bos sifre!")

    st.stop()

# =====================================================================
# TEMIZ VE TEKIL YAN MENU
# =====================================================================
with st.sidebar:
    st.image("https://ersanmakina.net/wp-content/uploads/2023/01/logo-ersan.png", use_container_width=True)
    
    role_map = {"admin": "Yonetici", "dealer": "Satici Bayi", "manufacturer": "Uretici"}
    r_text = role_map.get(st.session_state.user_role, "Kullanici")
    ...
    # Ikonlar metin kodu olarak girildi
    menu_items = [":house: Dashboard", ":page_facing_up: Yeni Teklif Hazirla", ":busts_in_silhouette: Musterilerim", ":clipboard: Gecmis Tekliflerim", ":gear: Profil Ayarlarim"]
    if st.session_state.user_role == "admin":
        menu_items.extend([":office: Bayi Yonetimi", ":package: Tum Modelleri Yonet"])
        
    menu = st.radio("SISTEM MENUSU", menu_items, label_visibility="collapsed")
...
if menu == ":house: Dashboard":
    ...
    if st.session_state.user_role == "admin":
        ...
        sold_val = df_dash[df_dash["Durum"] == "Onaylandi"]["Tutar"].sum()
        c3.markdown(f'<div class="stat-card" style="border-left-color:#10b981;"><span class="stat-title">Gerceklesen Satis</span><span class="stat-val">{sold_val:,.2f}</span></div>', unsafe_allow_html=True)
        
        dealer_count = get_user_query("SELECT COUNT(*) FROM users WHERE role='dealer'")[0][0]
        c4.markdown(f'<div class="stat-card" style="border-left-color:#6366f1;"><span class="stat-title">Aktif Bayi Sayisi</span><span class="stat-val">{dealer_count}</span></div>', unsafe_allow_html=True)
        ...
        with col_status:
            st.subheader(":clipboard: Durum Dagilimi")
            if not df_dash.empty:
                st.bar_chart(df_dash["Durum"].value_counts())

        st.subheader(":arrows_counterclockwise: Son Tekliflerin Durumunu Yonet")
        ...
        for oid, cust, mod, price, stat in recent_offers:
            with st.container(border=True):
                ...
                new_stat = cb.selectbox("Durum", ["Beklemede", "Onaylandi", "Reddedildi"], index=["Beklemede", "Onaylandi", "Reddedildi"].index(stat if stat in ["Beklemede", "Onaylandi", "Reddedildi"] else "Beklemede"), key=f"stat_{oid}", label_visibility="collapsed")
                if cc.button("Guncelle", key=f"btn_{oid}", use_container_width=True):
                    database.exec_query("UPDATE offers SET status=? WHERE id=?", (new_stat, oid))
                    st.toast(f"Teklif #{oid} guncellendi!")
                    st.rerun()
    else:
        st.info("Bayi portalina hos geldiniz. Sol menuden islemlere baslayabilirsiniz.")

elif menu == ":gear: Profil Ayarlarim":
    st.header(":gear: Kurumsal Profil Ayarlari")
    ...
    with st.form("p_form"):
        c1, c2 = st.columns(2)
        p_name = c1.text_input("Firma Adi", value=u_data[0])
        ...
        p_adr = st.text_area("Acik Adres", value=u_data[4] if u_data[4] else "")
        up_logo = st.file_uploader("Kurumsal Logo (PNG/JPG)", type=['png','jpg','jpeg'])
        if st.form_submit_button(":white_check_mark: GUNCELLE", type="primary"):
            f_logo = u_data[5]
            if up_logo:
            ...
            st.success("Profil guncellendi!")
            st.rerun()

elif menu == ":page_facing_up: Yeni Teklif Hazirla":
    offer_wizard.show_offer_wizard(st.session_state.user_id, st.session_state.user_role == "admin")

elif menu == ":busts_in_silhouette: Musterilerim":
    customer_pages.show_customer_management(st.session_state.user_id, st.session_state.user_role == "admin")

elif menu == ":clipboard: Gecmis Tekliflerim":
    st.header(":clipboard: Gecmis Tekliflerim")
...
elif menu == ":package: Tum Modelleri Yonet":
    model_management.show_product_management()

elif menu == ":office: Bayi Yonetimi":
    st.header(":office: Uyelik Onay ve Yonetim Sistemi")
    ...
    if pending:
        for p_id, p_name, p_type, p_email, p_phone in pending:
            with st.container(border=True):
                ...
                if col2.button(":white_check_mark: Sistemi Ac", key=f"app_{p_id}", use_container_width=True):
                    exec_user_query("UPDATE users SET is_approved=1 WHERE id=?", (p_id,))
                    st.rerun()
    else: st.info("Onay bekleyen basvuru yok.")
