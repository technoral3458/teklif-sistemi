...
def show_form_view(mode="add", mod_id=None, user_role="dealer"):
    ...
    # =====================================================================
    # DONANIM RESİMLERİNİ KART HALİNDE LİSTELEME
    # =====================================================================
    with tab_donanim:
        ...
        for idx, opt in enumerate(opts_avail):
            ...
            with chk_cols[idx % 3]:
                with st.container(border=True):
                    img_b64 = get_image_base64(o_img)
                    if img_b64:
                        st.markdown(f'<div style="text-align:center;"><img src="{img_b64}" style="width:100%; height:80px; object-fit:contain; margin-bottom:10px;"></div>', unsafe_allow_html=True)
                    else:
                        st.markdown(f"<div style='height:80px; display:flex; align-items:center; justify-content:center; background:#f1f5f9; border-radius:4px; color:#94a3b8; font-size:11px; margin-bottom:10px;'>{_m('no_img')}</div>", unsafe_allow_html=True)
                    
                    if st.checkbox(f"{o_name} {p_text}".strip(), value=is_checked, key=f"chk_{o_id}"): 
