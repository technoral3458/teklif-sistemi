...
def repair_factory_db():
    ...
    cols_options = [('allow_qty', "INTEGER DEFAULT 1"), ('opt_name_zh', "TEXT DEFAULT ''"), ('opt_desc_zh', "TEXT DEFAULT ''"), ('user_id', "INTEGER DEFAULT 1")]
    for col, typ in cols_options:
        try: exec_factory(f"ALTER TABLE options ADD COLUMN {col} {typ}")
        except: pass
...
# =====================================================================
# LİSTELEME VE VİTRİN (İzolasyon Filtreli)
# =====================================================================
                        ...
                        for j in range(4):
                            if i + j < len(cat_mods):
                                ...
                                with cols[j].container(border=True):
                                    ...
                                    with btn_c2:
                                        if st.button(_m("btn_copy"), key=f"mc_{row['id']}", use_container_width=True):
                                    ...
                                    with btn_c3:
                                        if st.button(_m("btn_del"), key=f"md_{row['id']}", use_container_width=True):
                ...
                for j in range(4):
                    if i + j < len(opts):
                        ...
                        with cols[j].container(border=True):
                            ...
                            with btn_c2:
                                if st.button(_m("btn_copy"), key=f"oc_{o_id}", use_container_width=True):
                                    ...
                                    st.rerun()
                            with btn_c3:
                                if st.button(_m("btn_del"), key=f"od_{o_id}", use_container_width=True):
            ...
            if not st.session_state.o_name: st.error(_m("err_opt_name"))
                ...
                with st.spinner(_m("translating")):
                    ...
                    if user_role == "manufacturer":
                        ...
                        else: exec_factory("INSERT INTO options (opt_name, opt_name_zh, opt_desc, opt_desc_zh, opt_price, opt_image, allow_qty, user_id) VALUES (?,?,?,?,?,?,?,?)", (final_opt_name_tr, final_opt_name_zh, final_opt_desc_tr, final_opt_desc_zh, st.session_state.o_price, st.session_state.o_img, allow_q, user_id))
                    else:
                        ...
                        else: exec_factory("INSERT INTO options (opt_name, opt_desc, opt_price, opt_image, allow_qty, user_id) VALUES (?,?,?,?,?,?)", (final_opt_name_tr, final_opt_desc_tr, st.session_state.o_price, st.session_state.o_img, allow_q, user_id))
                    st.session_state.view_mode = "list"; st.rerun()
