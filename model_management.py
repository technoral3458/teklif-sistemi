...
def auto_translate_to_tr(text):
    """Verilen metni otomatik olarak Türkçe'ye çevirir."""
...
# =====================================================================
# FABRİKA VERİTABANI BAĞLANTILARI VE TAMİRCİSİ (ÇİFT DİL DESTEĞİ EKLENDİ)
# =====================================================================
...
def repair_factory_db():
    ...
    # Yeni eklenen Çift Dil (Orijinal) Sütunları
    try:
...
def show_product_management():
    ...
    if not TRANSLATOR_READY and st.session_state.get("user_role") == "manufacturer":
        st.warning("⚠️ Çeviri modülü (deep-translator) kurulu değil. Lütfen sunucunuza kurun: `pip install deep-translator`")
...
def show_list_view(user_role):
    # NOKTA ATIŞI CSS (SADECE 3 BUTONU MOBİLDE YANYANA KİTLER)
    st.markdown("""
    <style>
    ...
    with tab_mod:
        ...
        st.markdown("---")
        # Listede her zaman ana dili (Türkçe) ismini çekiyoruz. Üreticiye Çince hali form içinde gösterilecek.
                    ...
                    for i in range(0, len(cat_mods), 4):
                        ...
                        for j in range(4):
                            if i + j < len(cat_mods):
                                ...
                                # Üreticiye listede kendi girdiği Çince ismi göster, diğerlerine Türkçe.
...
# =====================================================================
# MAKİNE EKLEME/DÜZENLEME FORMU (ÇİFT DİL AKILLI YÜKLEME)
# =====================================================================
def show_form_view(mode="add", mod_id=None, user_role="dealer"):
    ...
    if not st.session_state.get("form_loaded", False):
        ...
        if is_edit:
            ...
            # Üreticiyse orijinal dilini (Çince) yükle, Adminse çevrilmişi (Türkçe) yükle
    ...
    with tab_donanim:
        # Üretici eklerken resimli donanım listesi
    ...
    if st.button(btn_save_text, type="primary", use_container_width=True, key="btn_save_machine"):
        ...
        if not st.session_state.f_name: 
            ...
            with st.spinner(_m("translating")):
                ...
                # YAPAY ZEKA ÇEVİRİ VE KAYIT ALANI
                if user_role == "manufacturer":
                    ...
                    # Yönetici kaydediyorsa, varsayılan dile (Türkçe'ye) kaydeder.
                    final_name_tr = st.session_state.f_name
                    final_name_zh = st.session_state.get('f_name_zh', '') # Eski Çinceyi bozmaz
                    ...
                    final_specs_tr = " || ".join(spec_strs_tr) + (" || " if spec_strs_tr else "")
                    final_specs_zh = st.session_state.get('f_specs_zh', '') # Eski Çinceyi bozmaz
...
# =====================================================================
# DONANIM EKLEME/DÜZENLEME FORMU (ÇİFT DİL DESTEKLİ)
# =====================================================================
